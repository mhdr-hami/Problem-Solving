...
It's natural to consider the approach of `brute force` when faced with this problem. Since the input array is not sorted, we need to scan the entire array for each element to find the other element that makes up the target. As a result, the time complexity of this approach would be $O(n^2)$. As we move on to the third step of my general approach, it becomes clear that this approach is bound to result in a time-limit error. Additionally, we have to keep track of the indexes of elements, which means any operation that moves and swaps the elements is a challenge. <br> Following the `brute force` idea, it would be great if we didn't have to walk through the entire array for each element  ($O(n^2)$). What does it require to find the answer only in one pass over the array ($O(n)$)? It means for each element, you are only allowed to do one comparision. So the problem is which other element to choose to make the comparision between their sum and the `target`. How can we guarantee that comparision is the only right one to do? The problem is array not being sorted. If it was, we could use two pointers, one at the beginning and one at the end of the array. After comparing their sum with the `target` we could be sure what is next best pair to consider. For instance, if their sum is less than `target`, it means we need to consider a pair with a larger sum, and the by moving the small pointer one step to the right, we are sure we achieved the next larger sum available in the entire array. <br> But, sorting an array of elements can be difficult if you also need to keep track of their indexes. However, there is a simple solution to this problem. If you require both sorting and index tracking, you can pair each element with its index and then sort the resultant array of pairs. 
