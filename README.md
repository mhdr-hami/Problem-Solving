...
The memoization method is exactly the recursive function, except we store the numbers in `dp[]` before returning them, and check for answers in `dp[]` before calculating them. Knowing this, its much easier to come up with the DP secret: <br> `if(text1[i-1]==text2[j-1])dp[i][j] = 1 + dp[i-1][j-1]` <br> `else dp[i][j] = max(dp[i-1][j], dp[i][j-1])`
